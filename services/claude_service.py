...
async def analyze_market_final(
    symbol: str,
    indicators: Dict[str, Any],
    current_price: float,
    portfolio: Dict[str, Any],
    order_book: Optional[Dict] = None,
    ind_1h: Optional[Dict] = None,
    btc_context: Optional[str] = None,
    retry_count: int = 0,
) -> Dict[str, Any]:
    """
    Final validation using Claude Sonnet — called once for the best candidate signal.
    Richer prompt: order book imbalance + 1h timeframe data + BTC macro context.
    Falls back to Haiku analyze_market on any Sonnet failure.
    """
    safe_portfolio = {
        "available_usdt": float(portfolio.get("available_usdt", 0)),
        "total_pnl_pct":  float(portfolio.get("total_pnl_pct", 0)),
    ...
    # Order book section
    ob_section = ""
    if order_book and order_book.get("imbalance_signal") != "NEUTRAL":
        ...
        ob_section = (
            f"\nORDER BOOK : ratio bid/ask={ob_ratio:.2f} | pression={ob_signal} ({ob_score:.0%})"
            f"\n  Mur achat @ {order_book.get('bid_wall_price',0):.4f}"
            f"  Mur vente @ {order_book.get('ask_wall_price',0):.4f}"
        )

    # 1h context section
    h1_section = ""
    if ind_1h:
        rsi_1h   = ind_1h.get("momentum", {}).get("rsi", 50)
        macd_1h  = ind_1h.get("trend", {}).get("macd_histogram", 0)
        adx_1h   = ind_1h.get("trend", {}).get("adx", 0)
        ema9_1h  = ind_1h.get("trend", {}).get("ema_9", 0)
        ema21_1h = ind_1h.get("trend", {}).get("ema_21", 0)
        if ema9_1h > 0 and ema21_1h > 0:
            trend_1h = "haussier" if (ema9_1h > ema21_1h and macd_1h > 0) else \
                       "baissier" if (ema9_1h < ema21_1h and macd_1h < 0) else "neutre"
            h1_section = (
                f"\nCONTEXTE 1H : RSI={rsi_1h:.1f} | ADX={adx_1h:.1f} | "
                f"EMA9{'>' if ema9_1h > ema21_1h else '<'}EMA21 | tendance={trend_1h}"
            )

    btc_section = f"\nBTC MACRO : {btc_context}" if btc_context else ""

    prompt = f"""VALIDATION FINALE — {symbol} — Signal technique {r_action}

PRIX : ${current_price:.4f}  |  Capital : ${safe_portfolio['available_usdt']:.2f}  |  WinRate : {safe_portfolio['win_rate']:.0f}%

SIGNAL 5m : {r_action} (score={r_score}/15, conf={r_conf:.0%})
{r_reason}

INDICATEURS 5m :
RSI={rsi:.1f} {'[SURVENDU]' if rsi < 35 else '[SURACHETÉ]' if rsi > 65 else ''} | MACD={macd_h:.6f} | ADX={adx:.1f}
EMA9={ema9:.4f} {'>' if ema9 > ema21 else '<'} EMA21={ema21:.4f} | BB_pct={bb_pct:.2f}
Volume={vol_r:.1f}x | CMF={cmf:.3f} | Support={support:.4f} | Résist={resistance:.4f}
Patterns: {', '.join(patterns) if patterns else 'Aucun'}{ob_section}{h1_section}{btc_section}

RÈGLES :
- BUY score>=5 + OB acheteur + 1h haussier → BUY haute confiance
- Contradiction majeure (1h baissier ET OB vendeur) → HOLD
- score>=6 sans contradiction → confirme TOUJOURS
Retourne UNIQUEMENT le JSON."""
    ...
    try:
        ...
        if result:
            validated = _validate(result)
            if validated["action"] == "HOLD" and r_action != "HOLD" and r_score >= 6:
                logger.info(f"Sonnet HOLD overridden: {r_action} score={r_score}")
                validated["action"]     = r_action
                validated["confidence"] = max(validated["confidence"], r_conf)
                validated["key_factors"].insert(0, f"Rule override score={r_score}")
            validated["source"] = "claude_sonnet"
            return validated
        ...
        if retry_count < MAX_RETRY:
            await asyncio.sleep(20)
            return await analyze_market_final(
                symbol, indicators, current_price, portfolio,
                order_book, ind_1h, btc_context, retry_count + 1,
            )
        logger.warning("Sonnet rate limit — falling back to Haiku")
        return await analyze_market(symbol, indicators, current_price, portfolio)

    except Exception as e:
        logger.warning(f"Sonnet error: {e} — falling back to Haiku")
        return await analyze_market(symbol, indicators, current_price, portfolio)
